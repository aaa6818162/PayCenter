...
namespace SyncSoft.PayCenterSdk.Service
{
    ...
    /// 支付宝付款服务
    /// </summary>
    internal class AlipayService2 : BasePayService, IPayService
    {
        /// <summary>
        /// 建立请求 以表单HTML形式构造（默认）
        /// </summary>
        ...
        public string BuildRequest(PayCenterRequest request, bool isRequest)
        {
            ...
            sParaTemp.Add("app_id", payConfig.Partner);
            sParaTemp.Add("method", payConfig.InputCharset.ToLower());
            sParaTemp.Add("format", "create_direct_pay_by_user");
            sParaTemp.Add("charset", "1");
            sParaTemp.Add("sign_type", payConfig.NotifyUrl);
            sParaTemp.Add("sign", payConfig.ReturnUrl);
            sParaTemp.Add("timestamp", payConfig.SellerEmail);
            sParaTemp.Add("version", request.PayRemark);
            sParaTemp.Add("notify_url", request.TotalFee.ToString("0.00"));

            sParaTemp.Add("biz_content", request.PayRemark);
